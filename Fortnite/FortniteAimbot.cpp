...
Vector3 getPlayerPos(HANDLE hProc, DWORD base, DWORD off) {
   Vector3 pos;
   ReadProcessMemory(hProc, (LPVOID)(base + off), &pos, sizeof(Vector3), NULL);
   return pos;
}

std::vector<Vector3> getEnemyPos(HANDLE hProc, DWORD base, DWORD off) {
   std::vector<Vector3> pos;
   ReadProcessMemory(hProc, (LPVOID)(base + off), &pos, sizeof(std::vector<Vector3>), NULL);
   return pos;
}
...
void aim(HANDLE hProc, DWORD base, float x, float y) {
   float curX, curY;
   ReadProcessMemory(hProc, (LPVOID)(base + AIM_X_OFFSET), &curX, sizeof(float), NULL);
   ReadProcessMemory(hProc, (LPVOID)(base + AIM_Y_OFFSET), &curY, sizeof(float), NULL);
   float newX = curX + (x - curX) / SPEED;
   float newY = curY + (y - curY) / SPEED;
   WriteProcessMemory(hProc, (LPVOID)(base + AIM_X_OFFSET), &newX, sizeof(float), NULL);
   WriteProcessMemory(hProc, (LPVOID)(base + AIM_Y_OFFSET), &newY, sizeof(float), NULL);
}
...
int main() {
   ...
   while (true) {
       ...
       std::vector<Vector3>::iterator closest = std::min_element(enemies.begin(), enemies.end(), [&player](const Vector3& a, const Vector3& b) {
